...
/* Load CSS */
// FIXME, only on frontend when active....
function gwolle_gb_frontend_css() {
	//wp_enqueue_style('gwolle_gb_frontend_css', GWOLLE_GB_DIR . '/frontend/style.css', 'screen');
	wp_enqueue_style('gwolle_gb_frontend_css', plugins_url('frontend/style.css', __FILE__), 'screen');
}
add_action('wp_enqueue_scripts', 'gwolle_gb_frontend_css');

// FIXME: don't use the PHP4 version anymore, and use the main plugin, not an addon.
...
function gwolle_gb_init() {
	...
	switch($gwolle_gb_function) {
		...
		case 'edit_entry' :
			...
			$entry = gwolle_gb_get_entries(array('entry_id' => $entry_id));
			if ($entry === FALSE) {
