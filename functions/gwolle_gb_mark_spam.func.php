...
if (!function_exists('gwolle_gb_mark_spam')) {
	function gwolle_gb_mark_spam($args = array()) {
		...
		$entry = gwolle_gb_get_entries(array('entry_id' => $args['entry_id']));
		if ($entry === FALSE) {
			return FALSE;
		}
		...
		if (version_compare(phpversion(), '5.0', '>=')) {//	Use the PHP5 class
			...
			$comment = array('author' => $entry['entry_author_name'], 'email' => $entry['entry_author_email'], 'website' => $entry['entry_author_website'], 'body' => $entry['entry_content'], 'permalink' => get_bloginfo('wpurl'));
