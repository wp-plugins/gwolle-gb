<?php
if (!function_exists('gwolle_gb_check_entry_data')) {
	/**
	 * gwolle_gb_check_entry_data
	 * Checks the data of an entry
	 * Returns:
	 * - $entry   normalized entry data if no errors were found
	 * - FALSE    if any errors where found.
	 */
	function gwolle_gb_check_entry_data($args = array()) {
		global $wpdb;
		global $current_user;
		...
		if ($action == 'update') {
			...
			if (strlen($entry['content']) == 0) {
				$error_messages[] = __('Please write an entry.', GWOLLE_GB_TEXTDOMAIN);
			...
			if ($gwolle_gb_settings['recaptcha-active'] === TRUE) {
				if (!function_exists('recaptcha_get_html')) {
					...
					require_once (GWOLLE_GB_DIR . '/frontend/recaptcha/recaptchalib.php');
				}
				$privatekey = get_option('recaptcha-private-key');
				$resp = recaptcha_check_answer($privatekey, $entry['ip'], $_POST["recaptcha_challenge_field"], $_POST["recaptcha_response_field"]);
				if (!$resp -> is_valid) {
					$error_messages[] = __('The captcha has not been entered correctly.', GWOLLE_GB_TEXTDOMAIN);
				}
			}

			// Check if the 'name' field is empty
			if (strlen($entry['name']) == 0) {
				$error_messages[] = __('Please enter a name.', GWOLLE_GB_TEXTDOMAIN);
				$error_fields[] = 'name';
			}
        ...
        LIMIT 1";
			...
			if ($result > 0) {
				...
				$error_messages[] = __('Double post: An entry with the data you entered has already been saved.', GWOLLE_GB_TEXTDOMAIN);
			}
