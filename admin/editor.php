<?php
	...
	*/
	...
	include_once(GWOLLE_GB_DIR.'/functions/gwolle_gb_output_to_input_field.func.php');
                ...
                <div id="minor-publishing">
                  <div id="misc-publishing-actions">
                    <div class="misc-pub-section misc-pub-section-last">
		                    ...
		                    <input id="entry_isChecked" name="entry_isChecked" type="checkbox" <?php if ($entry['entry_isChecked'] == '1') { echo 'checked="checked"'; } ?> /> <?php _e('This entry is checked.',GWOLLE_GB_TEXTDOMAIN); ?>
		                  </label>
										</div>
                ...
                <div id="major-publishing-actions">
                  <div id="publishing-action">
                    ...
                    <?php if ($entry['entry_id']) { ?>
											<a class="submitdelete deletion" href="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo GWOLLE_GB_FOLDER; ?>/entries.php&amp;gwolle_gb_function=trash_entry&amp;entry_id=<?php echo $entry['entry_id']; ?>"><?php _e('Trash'); ?></a>
											&nbsp;
											<?php
												if ($entry['entry_isSpam'] == '0') {
											?>
													<a class="submitdelete deletion" href="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo GWOLLE_GB_FOLDER; ?>/editor.php&amp;gwolle_gb_function=mark_spam&amp;entry_id=<?php echo $entry['entry_id']; ?>&amp;show=editor" onClick="return confirm('<?php _e("You\'re about to mark this guestbook entry as spam. It will be sent to the Akismet team to help other people fighting spam. Entries marked as spam are automatically deleted after 15 days. Continue?",GWOLLE_GB_TEXTDOMAIN); ?>');"><?php _e('Spam!',GWOLLE_GB_TEXTDOMAIN); ?></a>
											<?php
												}
												else {
											?>
													<a class="submitdelete deletion" href="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo GWOLLE_GB_FOLDER; ?>/editor.php&amp;gwolle_gb_function=unmark_spam&amp;entry_id=<?php echo $entry['entry_id']; ?>&amp;show=editor" onClick="return confirm('<?php _e("A message will be sent to the Akismet team that this entry is not spam. Continue?",GWOLLE_GB_TEXTDOMAIN); ?>');"><?php _e('No Spam!',GWOLLE_GB_TEXTDOMAIN); ?></a>
											<?php
												}
											}
										?>
          ...
          <div id="gwolle_gb-entry-details" class="postbox " >
            ...
            <div class="inside">
              <div class="tagsdiv" id="post_tag">
                <?php _e('Author',GWOLLE_GB_TEXTDOMAIN); ?>: <span><?php if ($entry['entry_author_name']) { echo stripslashes(htmlentities($entry['entry_author_name'])); } else { echo '<strong>' . __('You',GWOLLE_GB_TEXTDOMAIN) . '</strong>'; } ?></span>
								<br><br>
								<?php _e('E-Mail',GWOLLE_GB_TEXTDOMAIN); ?>: <span><?php if (strlen(str_replace(' ','',$entry['entry_author_email'])) > 0) { echo stripslashes(htmlentities($entry['entry_author_email'])); } else { echo '<i>(' . __('unknown',GWOLLE_GB_TEXTDOMAIN) . ')</i>'; } ?></span>
								<br><br>
								<?php _e('Written',GWOLLE_GB_TEXTDOMAIN); ?>: <span><?php if ($entry['entry_date'] > 0) { echo date('d.m.Y, H:i',$entry['entry_date']) . ' ' . __("o'clock",GWOLLE_GB_TEXTDOMAIN); } else { echo '(' . __('not yet',GWOLLE_GB_TEXTDOMAIN) . ')'; } ?></span>
								<br><br>
								<?php _e("Author's IP-address",GWOLLE_GB_TEXTDOMAIN); ?>: <span><?php if (strlen($entry['entry_author_ip']) > 0) { echo '<a href="http://www.db.ripe.net/whois?form_type=simple&searchtext='.$entry['entry_author_ip'].'" title="'.__('Whois search for this IP',GWOLLE_GB_TEXTDOMAIN).'" target="_blank">'.$entry['entry_author_ip'].'</a>'; } else { echo '<i>(' . __('unknown',GWOLLE_GB_TEXTDOMAIN) . ')</i>'; } ?></span>
								<br><br>
								<?php _e('Host',GWOLLE_GB_TEXTDOMAIN); ?>: <span><?php if (strlen($entry['entry_author_host']) > 0) { echo $entry['entry_author_host']; } else { echo '<i>(' . __('unknown',GWOLLE_GB_TEXTDOMAIN) . ')</i>'; } ?></span>
              </div>
            </div>
          </div><!-- Seitenbox-Ende -->
        ...
        <div id="post-body-content">
          <div id='normal-sortables' class='meta-box-sortables'>
            <div id="authordiv" class="postbox " >
              <div class="handlediv" title="Klicken zum Umschalten"><br /></div><h3 class='hndle'><span><?php _e('Guestbook entry',GWOLLE_GB_TEXTDOMAIN); ?></span></h3>
              <div class="inside">
                ...
                <?php if (get_option('gwolle_gb-showLineBreaks')=='false') { echo '<p>' . str_replace('%1','admin.php?page='.GWOLLE_GB_FOLDER.'/settings.php',__('Line breaks will not be visible to the visitors due to your <a href="%1">settings</a>.',GWOLLE_GB_TEXTDOMAIN)) . '</p>'; } ?>
              </div>
            </div>
            <div id="authordiv" class="postbox " >
              <div class="handlediv" title="Klicken zum Umschalten"><br /></div><h3 class='hndle'><span><?php _e('Homepage',GWOLLE_GB_TEXTDOMAIN); ?></span></h3>
              <div class="inside">
            ...
            <div id="authordiv" class="postbox " >
              <div class="handlediv" title="Klicken zum Umschalten"><br /></div><h3 class='hndle'><span><?php _e('Origin',GWOLLE_GB_TEXTDOMAIN); ?></span></h3>
              <div class="inside">
