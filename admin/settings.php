...
<div class="wrap">
  ...
  ?>
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo GWOLLE_GB_FOLDER; ?>/settings.php&amp;action=saveSettings">
        ...
        <tr valign="top">
            ...
            <?php
              ...
              echo '<img id="post_id_status" style="height:10px;" src="'.GWOLLE_GB_URL.'/admin/gfx/'.$post_id_img.'" alt="'.__('Not set.', GWOLLE_GB_TEXTDOMAIN).'" />';
						...
						<span class="setting-description"><?php _e('This ID is used to generate the correct link to the guestbook read/write page at the frontend.',GWOLLE_GB_TEXTDOMAIN); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td>
						<input <?php if ($gwolle_gb_settings['moderate-entries'] === TRUE) { echo 'checked="checked"'; } ?> type="checkbox" name="moderate-entries" id="moderate-entries"> <?php _e('Moderate entries before publishing them.',GWOLLE_GB_TEXTDOMAIN); ?>
						<br />
				...
				<tr valign="top">
					<th scope="row"><label for="access_control"><?php _e('Access control',GWOLLE_GB_TEXTDOMAIN); ?></label></th>
					<td>
						...
						<br />
						<span class="setting-description"><?php _e('Choose the userlevel that has access to the guestbook backend.',GWOLLE_GB_TEXTDOMAIN); ?></span>
					</td>
				</tr>
					...
					<td>
						<?php
							...
							if (!function_exists('mail')) {
								echo '<span class="setting-description">' . __('Sorry, but the function <code>mail()</code> required to notify you by mail is not enabled in your PHP configuration. Please contact your hosting provider to change this.',GWOLLE_GB_TEXTDOMAIN) . '</span>';
							}
							else {
						?>
								...
								<span class="setting-description"><?php _e('E-Mail me no matter the new entry is spam or not.',GWOLLE_GB_TEXTDOMAIN); ?></span>
								
								<div>
									<?php
										_e('The following users have subscribed to this service:',GWOLLE_GB_TEXTDOMAIN);
				...
				<tr valign="top">
					...
					<td>
						<input name="recaptcha-active" <?php if ($gwolle_gb_settings['recaptcha-active'] === TRUE) { echo 'checked="checked" '; } ?>id="use-recaptcha" type="checkbox"> <?php _e('Use Recaptcha',GWOLLE_GB_TEXTDOMAIN); ?>
						<br />
				...
				<tr valign="top">
					<th scope="row"><label for="akismet-settings">Akismet</label><br /><span class="setting-description"><a href="http://akismet.com/" title="<?php _e('Learn more about Akismet...',GWOLLE_GB_TEXTDOMAIN); ?>" target="_blank"><?php _e("What's that?",GWOLLE_GB_TEXTDOMAIN); ?></a></span></th>
					<td>
						<?php
							...
							if (version_compare(phpversion(),'5.0','>=')) {
								$classUsed = str_replace('%1','http://www.achingbrain.net/stuff/php/akismet',__('Using the <a href="%1" target="_blank">Akismet PHP5-Class by Alex</a>.',GWOLLE_GB_TEXTDOMAIN));
							}
							elseif (version_compare(phpversion(),'4.0','>=')) {
								$classUsed = str_replace('%1','http://miphp.net/blog/view/php4_akismet_class',__('Using the <a href="%1" target="_blank">Akismet PHP4-Class by Bret Kuhns</a>.',GWOLLE_GB_TEXTDOMAIN));
							}
							...
							elseif (version_compare(phpversion(),'4.0','<')) {
								echo str_replace('%1',phpversion(),__("I'm sorry, but it seems you're running an PHP version prior 4.x. The Akismet classes that come with Gwolle-GB are only compatible to 4.x and higher. To be able to use Akismet you should consider updating your server. (The System says your PHP version is %1.)",GWOLLE_GB_TEXTDOMAIN));
							}
							//	Check if a Wordpress API key is defined and set in the database.
							elseif (!$wordpress_api_key) {
								echo str_replace('%1','plugins.php?page=akismet-key-config',__("Sorry, wasn't able to locate your <strong>Wordpress API key. You can enter it at the <a href=\"%1\">Akismet configuration page</a>.",GWOLLE_GB_TEXTDOMAIN));
							}
							//	The API key doesn't seem to be valid
							elseif ($apiKeyValid === FALSE) {
								echo str_replace('%1','plugins.php?page=akismet-key-config',__("There seems to be something wrong with your Wordpress API key; wasn't able to validate it correctly. Please check it via <a href=\"%1\">Akismet configuration page</a>.",GWOLLE_GB_TEXTDOMAIN));
							}
							else {
								//	Akismet is installed, PHP-Version is 4.x or higher and a Wordpress api key exists
								echo '<input '; if ($gwolle_gb_settings['akismet-active'] === TRUE) { echo 'checked="checked" '; } echo 'name="akismet-active" id="akismet-active" type="checkbox" /> ' . __('Use Akismet',GWOLLE_GB_TEXTDOMAIN) . ' <i>(' . $classUsed . ')</i>';
								echo '<br />';
								_e("I've found the Wordpress API key, so you can start using Akismet right now.",GWOLLE_GB_TEXTDOMAIN);
							}
						?>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td>
						<input type="checkbox" <?php if ($gwolle_gb_settings['showEntryIcons'] === TRUE) { echo 'checked="checked"'; } ?> name="showEntryIcons" /> <?php _e('Show entry icons',GWOLLE_GB_TEXTDOMAIN); ?>
						<br />
						<span class="setting-description"><?php _e('These icons are shown in every entry row, so that you know its entry status (spam, locked or unlocked).',GWOLLE_GB_TEXTDOMAIN); ?></span>
					</td>
				</tr>
						...
						<select name="entriesPerPage">
							<?php
								...
								for ($i=0; $i<count($presets); $i++) {
									echo '<option value="' . $presets[$i] . '"'; if ($presets[$i] == $entriesPerPage) { echo ' selected="selected"'; } echo '>' . $presets[$i] . ' ' . __('Entries',GWOLLE_GB_TEXTDOMAIN) . '</option>';
								}
							?>
						</select>
						<br />
						<span class="setting-description"><?php _e('Number of entries shown on the frontend in reading mode.',GWOLLE_GB_TEXTDOMAIN); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td>
						<input type="checkbox" id="showLineBreaks" name="showLineBreaks"<?php if ($gwolle_gb_settings['showLineBreaks'] === TRUE) { echo ' checked="checked"'; } ?> /> <label for="showLineBreaks"><?php _e('Show line breaks.',GWOLLE_GB_TEXTDOMAIN); ?></label>
						<br />
						...
						<br />
						<input type="checkbox" id="guestbookOnly" name="guestbookOnly"<?php if ($gwolle_gb_settings['guestbookOnly'] === TRUE) { echo ' checked="checked"'; } ?> /> <label for="guestbookOnly"><?php _e('Display the guestbook only, not the text around it.',GWOLLE_GB_TEXTDOMAIN); ?></label>
						<br />
						<span class="setting-description"><?php _e('If you wanted to have content prepended/appended to the guestbook, such as a warm "Welcome, please leave me an entry.", turn this off.',GWOLLE_GB_TEXTDOMAIN); ?></span>
						<br />
						<input type="checkbox" id="showSmilies" name="showSmilies"<?php if ($gwolle_gb_settings['showSmilies'] === TRUE) { echo ' checked="checked"'; } ?> /> <label for="showSmilies"><?php _e('Display smilies as images.',GWOLLE_GB_TEXTDOMAIN); ?></label>
						<br />
						...
						<br />
						<input type="checkbox" id="showSmilies" name="linkAuthorWebsite"<?php if ($gwolle_gb_settings['linkAuthorWebsite'] === TRUE) { echo ' checked="checked"'; } ?> /> <label for="showSmilies"><?php _e("Link authors' name to their website.",GWOLLE_GB_TEXTDOMAIN); ?></label>
						<br />
						<span class="setting-description"><?php echo str_replace('%1',convert_smilies(':)'),__("The author of an entry can set his/her website. If this setting is checked, his/her name will be linked to that website.",GWOLLE_GB_TEXTDOMAIN)); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td>
						...
						<span class="setting-description">
							<?php
								_e('You can set the content of the mail a notification subscriber gets on new entries. The following tags are supported:',GWOLLE_GB_TEXTDOMAIN);
								echo '<br />';
		    ...
		    <tr valign="top">
					...
					<td>
						<input <?php if ($gwolle_gb_settings['checkForImport'] === TRUE) { echo 'checked="checked"'; } ?> type="checkbox" name="checkForImport" id="checkForImport" /> <?php _e('Check if there are other guestbook plugins installed to import their entries.',GWOLLE_GB_TEXTDOMAIN); ?>
						<br />
						<span class="setting-description">
							<?php _e("If Gwolle-GB detects another guestbook plugin for WordPress it'll prompt you to import that guestbooks's data.",GWOLLE_GB_TEXTDOMAIN); ?>
						</span>
					</td>
				</tr>
		...
		<table style="margin-top:30px;" class="form-table">
			<tr valign="top" style="margin-top:30px;">
				...
				<td>
					<?php
						_e('Uninstalling means that all database entries are removed (settings and entries).',GWOLLE_GB_TEXTDOMAIN);
						echo '<br />';
						_e('This can <strong>not</strong> be undone.',GWOLLE_GB_TEXTDOMAIN);
					?>
					<br />
					<a style="color:#ff0000;" href="admin.php?page=<?php echo GWOLLE_GB_FOLDER; ?>/settings.php&setting_page=uninstall"><?php _e("I'm aware of that, continue!",GWOLLE_GB_TEXTDOMAIN); ?> &raquo;</a>
				</td>
			</tr>
		</table>
	...
	<?php
			...
			str_replace('%1',$_SERVER['PHP_SELF'] . '?page='.GWOLLE_GB_FOLDER.'/settings.php',__('Sorry, but the page you\'re looking for does not exists. Go back to the <a href="%1">settings page</a>', GWOLLE_GB_TEXTDOMAIN));
		}
	?>
