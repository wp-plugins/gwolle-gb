...
if (!current_user_can('moderate_comments')) {
	...
	if ($entry_id !== FALSE) {
		...
		if ($entry === FALSE) {
			...
			if ($entry['entry_content'] != $entry_content || $entry['entry_author_origin'] != $entry_author_origin || $entry['entry_author_website'] != $entry_author_website) {
				$result = $wpdb->query("
					UPDATE
						" . $wpdb->prefix . "gwolle_gb_entries e
					SET
						e.entry_content = '" . $_POST['entry_content'] . "',
						e.entry_author_origin = '" . $_POST['entry_author_origin'] . "',
						e.entry_author_website = '" . $_POST['entry_author_website'] . "'
					WHERE
						e.entry_id = " . $entry_id . "
					LIMIT 1
				");
				if ($result === 1) {
					...
					gwolle_gb_add_log_entry(array('subject' => 'entry-edited', 'subject_id' => $entry_id));
					$msg = 'updated=true';
				} else {
					$msg = 'updated=true';
				}
		...
		// FIXME, not all fields are being saved
		$result = $wpdb->query("
				INSERT
				INTO
					" . $wpdb->prefix . "gwolle_gb_entries e
		...
		header('Location: ' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=gwolle-gb/editor.php&entry_id=' . $wpdb->insert_id . '&' . $msg);
	}
}
?>
